...
# for creating lienar space, i took the help of ChatGPT3
...
for itr, num_samples in enumerate(P_samples):
    ...
    ax.plot(n_dim_lst,min_obj_rounded , 'g')
    ax.scatter(n_dim_lst,min_obj_rounded)
    ax.set_yscale('log')
    ax.set_xlabel('N no. of dimensions', fontsize=12)
    ax.set_ylabel('min. objective value', fontsize=10)
    ax.set_title(f'Random Sampling\nNumber of Samples = {num_samples}')
    for i, txt in enumerate(zip(n_dim_lst, min_obj_rounded)):
        ax.annotate(txt, (n_dim_lst[i], min_obj_rounded[i]))
...
for itr, num_samples in enumerate(P_samples):
    ...
    ax.set_xlabel('N no. of dimensions', fontsize=12)
    ax.set_ylabel('min. objective value', fontsize=10)
    ax.set_title(f'Uniform Sampling \nNumber of Samples = {num_samples}')
    for i, txt in enumerate(zip(n_dim_lst, min_obj_rounded)):
        ax.annotate(txt, (n_dim_lst[i], min_obj_rounded[i]))
